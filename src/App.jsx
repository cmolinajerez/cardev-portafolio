...
const PortafolioCarDev = () => {
  ...
  // ✅ NUEVO: Control del video del avatar
  ...
  const handleVideoControl = () => {
    ...
    if (videoState === 'initial' || videoState === 'ended') {
      ...
      avatarVideoRef.current.onended = () => setVideoState('ended');
  ...
  // ✅ NUEVO: Función para obtener el contenido del botón según estado
  const getButtonContent = () => {
    switch (videoState) {
      case 'initial': return { icon: '▶', text: 'Ver mi presentación' };
      case 'playing': return { icon: '⏸', text: 'Pausar video' };
      case 'paused': return { icon: '▶', text: 'Continuar video' };
      case 'ended': return { icon: '🔄', text: 'Ver de nuevo' };
      default: return { icon: '▶', text: 'Reproducir' };
    }
  };
  ...
  const speak = async (text, messageIndex, voiceMode) => {
    try {
      ...
      if (voiceMode === 'browser') {
        ...
        const preferredVoices = ['Google español', 'Microsoft Helena', 'Mónica', 'Paulina', 'Google español de Estados Unidos', 'Microsoft Laura'];
        utterance.voice = voices.find(voice => preferredVoices.some(pref => voice.name.includes(pref)));
        if (!utterance.voice) {
...
Responde de forma conversacional y estratégica:`
      ...
      const errorMessage = { role: 'assistant', content: 'Disculpa, tuve un problema al procesar tu pregunta. ¿Podrías intentarlo de nuevo?' };
  ...
  // ✅ MODIFICADO: HolographicAvatar recibe videoRef como prop
        ...
        <div className="relative w-full h-full">
          <div className="relative w-full h-full rounded-full overflow-hidden border-2 border-cyan-400/50">
            ...
            <div className="absolute inset-0 overflow-hidden">
              <div className="absolute w-full h-1 bg-gradient-to-r from-transparent via-cyan-400 to-transparent opacity-60" style={{ animation: 'scan 3s linear infinite' }}></div>
            </div>
            {isTalking && <div className="absolute inset-0 bg-cyan-400/10" style={{ animation: 'glitch 0.3s infinite' }}></div>}
          </div>
            ...
            {[...Array(8)].map((_, i) => {
              ...
              return (
                <circle key={i} cx={x} cy={y} r="2" fill="#06b6d4" opacity="0.6">
                  <animate attributeName="r" values="1;3;1" dur="2s" begin={`${i * 0.25}s`} repeatCount="indefinite" />
          ...
          {isThinking && (
            <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 flex gap-2">
              {[...Array(3)].map((_, i) => <div key={i} className="w-2 h-2 bg-cyan-400 rounded-full" style={{ animation: `bounce 0.6s ease-in-out ${i * 0.1}s infinite` }}></div>)}
            </div>
          )}
          {isTalking && (
            <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 flex gap-1">
              {[...Array(4)].map((_, i) => <div key={i} className="w-1 bg-cyan-400 rounded-full" style={{ height: '16px', animation: `wave 0.6s ease-in-out ${i * 0.1}s infinite` }}></div>)}
            </div>
          )}
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      ...
      <nav className="relative z-10 px-6 py-4 backdrop-blur-sm border-b border-white/10">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="text-2xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">CarDev</div>
      ...
      <section className="relative z-10 px-6 py-20 max-w-6xl mx-auto">
        <div className="text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-bold mb-6 leading-tight">
            Carla Pamela<br />
            <span className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">Molina Jerez</span>
          </h1>
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-10">
            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-lg backdrop-blur-sm border border-white/10">
              <Brain className="w-5 h-5 text-cyan-400" /><span>LLMs • RAG • MCP</span>
            </div>
            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-lg backdrop-blur-sm border border-white/10">
              <Users className="w-5 h-5 text-purple-400" /><span>15 años docencia</span>
            </div>
            <div className="flex items-center gap-2 bg-white/5 px-4 py-2 rounded-lg backdrop-blur-sm border border-white/10">
              <Zap className="w-5 h-5 text-cyan-400" /><span>Liderazgo de Equipos</span>
            </div>
          </div>
          <div className="flex flex-wrap justify-center gap-4">
            <a href="#conversation" className="bg-gradient-to-r from-cyan-500 to-purple-500 px-8 py-4 rounded-lg font-semibold text-lg hover:shadow-lg hover:shadow-cyan-500/50 transition">Conversemos</a>
            <a href="#projects" className="border border-white/20 px-8 py-4 rounded-lg font-semibold text-lg hover:bg-white/5 transition">Ver proyectos</a>
          </div>
        </div>
      </section>
      ...
      <section id="about" className="relative z-10 px-6 py-20 max-w-7xl mx-auto">
        <h2 className="text-4xl font-bold mb-12 text-center">
          <span className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">Mi Ventaja Diferencial</span>
        </h2>
        <div className="grid md:grid-cols-3 gap-8">
          <div className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl border border-white/10 hover:border-cyan-500/50 transition">
            ...
            <p className="text-gray-300">Construcción end-to-end de agentes conversacionales con LLMs, RAG y MCP. Arquitecturas completas desde backend hasta deployment en la nube.</p>
          </div>
          <div className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl border border-white/10 hover:border-purple-500/50 transition">
            ...
            <p className="text-gray-300">Coordinación de equipos de hasta 15 personas. Experiencia abriendo proyectos complejos de integración desde cero en entornos educativos.</p>
          </div>
          <div className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl border border-white/10 hover:border-cyan-500/50 transition">
            <Award className="w-12 h-12 text-cyan-400 mb-4" />
            <h3 className="text-xl font-bold mb-3">Pedagogía Experto 1</h3>
            <p className="text-gray-300">15 años enseñando con certificación Experto 1. Capacidad única de explicar conceptos complejos y capacitar equipos en adopción tecnológica.</p>
          </div>
        </div>
      </section>
      ...
      <section id="projects" className="relative z-10 px-6 py-20 max-w-7xl mx-auto">
        <h2 className="text-4xl font-bold mb-12 text-center">
          <span className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">Proyectos Destacados</span>
        </h2>
        <div className="grid md:grid-cols-3 gap-8">
          <div className="bg-gradient-to-br from-cyan-500/10 to-purple-500/10 backdrop-blur-sm p-8 rounded-2xl border border-cyan-500/30 hover:border-cyan-500 transition group">
            ...
            <p className="text-gray-300 mb-6">Sistema conversacional con RAG para optimizar la inducción técnica en TrakCare. Arquitectura completa: FastAPI, Streamlit, PostgreSQL, Vector Store OpenAI. Reduce curva de aprendizaje de 4-6 años.</p>
          ...
          <div className="bg-gradient-to-br from-purple-500/10 to-cyan-500/10 backdrop-blur-sm p-8 rounded-2xl border border-purple-500/30 hover:border-purple-500 transition group">
            ...
            <p className="text-gray-300 mb-6">Demo funcional de automatización con RAG para consultas de recursos humanos. Responde preguntas sobre vacaciones, evaluaciones, beneficios y políticas empresariales con información contextualizada.</p>
            <div className="flex flex-wrap gap-2">
          ...
          <div className="bg-gradient-to-br from-cyan-500/10 to-purple-500/10 backdrop-blur-sm p-8 rounded-2xl border border-cyan-500/30 hover:border-cyan-500 transition group">
            ...
            <p className="text-gray-300 mb-6">Desarrollo web completo para emprendimiento del sector construcción. Diseño moderno, responsive y optimizado para conversión de clientes.</p>
            <div className="flex flex-wrap gap-2">
      ...
      <section id="conversation" className="relative z-10 px-6 py-20 max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold mb-4">
            <span className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">Conversemos</span>
          </h2>
          <p className="text-gray-300 text-lg">Pregúntame sobre mi experiencia, proyectos y cómo puedo ayudarte. Puedes escribir o usar tu voz.</p>
        </div>
        ...
        <div className="grid md:grid-cols-[320px_1fr] gap-8 items-start">
          <div className="flex flex-col items-center gap-6">
            ...
            {/* ✅ NUEVO: Botón de control de video */}
            <button
              onClick={handleVideoControl}
              className={`px-6 py-3 rounded-xl font-semibold transition flex items-center gap-2 ${
                videoState === 'initial' 
                  ? 'bg-gradient-to-r from-cyan-500 to-purple-500 hover:shadow-lg hover:shadow-cyan-500/50 animate-pulse' 
                  : 'bg-gradient-to-r from-cyan-500/80 to-purple-500/80 hover:from-cyan-500 hover:to-purple-500'
              }`}
            >
              <span className="text-xl">{getButtonContent().icon}</span>
          ...
          <div className="bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-xl rounded-3xl border border-white/20 overflow-hidden shadow-2xl relative">
            <div ref={chatContainerRef} className="h-[500px] overflow-y-auto p-6 space-y-4">
              {messages.length === 0 && (
                <div className="text-center text-gray-400 mt-32">
                  ...
                  <p className="text-sm">Pregúntame sobre mi experiencia, proyectos o cómo puedo ayudarte con IA.</p>
              ...
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] ${msg.role === 'user' ? 'bg-gradient-to-r from-cyan-500 to-purple-500 text-white p-4 rounded-2xl' : 'bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl'}`}>
                    <div className="flex items-start gap-3">
                      <div className="flex-1 p-4">{msg.content}</div>
                      {msg.role === 'assistant' && (
                        <div className="flex flex-col gap-2 mt-4 mr-3">
                          ...
                          >
                            <Volume2 className="w-4 h-4" /><span>⚡</span>
                          </button>
                          <button
                  ...
                  <div className="flex items-center justify-center gap-1 h-12">
                    {[...Array(20)].map((_, i) => (
                      <div
                        ...
                        style={{ height: '100%', animation: `wave-recording 0.8s ease-in-out ${i * 0.05}s infinite`, opacity: 0.8 }}
                      ></div>
                    ))}
                  </div>
                  <p className="text-cyan-100 text-sm text-center mt-4 font-medium">🎤 Habla tu pregunta... Al terminar, presiona "Detener"</p>
                </div>
              </div>
            )}
            ...
            <div className="border-t border-white/20 p-4 bg-white/5">
              <div className="flex gap-2">
                <button
                  onClick={toggleListening}
                  className={`p-3 rounded-xl transition ${isListening ? 'bg-gradient-to-r from-cyan-500 to-teal-500 shadow-lg shadow-cyan-500/30 animate-pulse' : 'bg-white/10 hover:bg-white/20 border border-white/20'}`}
              ...
              {!isListening && !isSpeaking && !isGeneratingAudio && (
                <p className="text-xs text-gray-400 mt-2 text-center">💡 Cada respuesta tiene dos opciones: <span className="text-white">⚡ Rápida</span> (instantánea) o <span className="text-cyan-400">🎙️ Premium</span> (mi voz clonada IA)</p>
              )}
              {isGeneratingAudio && <p className="text-xs text-yellow-400 mt-2 text-center animate-pulse">🎵 Generando mi voz clonada con IA... (vale la espera)</p>}
              {isSpeaking && !isGeneratingAudio && <p className="text-xs text-cyan-400 mt-2 text-center">🔊 Reproduciendo • Haz clic de nuevo para pausar</p>}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative z-10 px-6 py-20 max-w-7xl mx-auto">
        <h2 className="text-4xl font-bold mb-12 text-center">
          <span className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">Servicios</span>
        </h2>
        <div className="grid md:grid-cols-2 gap-8">
          <div className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl border border-white/10">
            <h3 className="text-2xl font-bold mb-4">Agentes IA Conversacionales</h3>
            <p className="text-gray-300 mb-4">Diseño e implementación de soluciones con LLMs, RAG y MCP para automatizar procesos y gestión del conocimiento en organizaciones.</p>
            <a href="https://mentor-ia.cl" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:text-cyan-300 flex items-center gap-2">Visitar Mentor-IA <ExternalLink className="w-4 h-4" /></a>
          </div>
          <div className="bg-white/5 backdrop-blur-sm p-8 rounded-2xl border border-white/10">
            <h3 className="text-2xl font-bold mb-4">Desarrollo Web & Analytics</h3>
            <p className="text-gray-300 mb-4">Creación de sitios web, integración de soluciones y análisis de comportamiento de usuarios para optimizar conversiones.</p>
            <a href="https://desarrollo-web.cl" target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-purple-300 flex items-center gap-2">Ver servicios web <ExternalLink className="w-4 h-4" /></a>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="relative z-10 px-6 py-20 max-w-4xl mx-auto text-center">
        <h2 className="text-4xl font-bold mb-6">
          <span className="bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">¿Trabajamos juntos?</span>
        </h2>
        <p className="text-xl text-gray-300 mb-8">Estoy disponible para proyectos de implementación de IA, desarrollo web y consultoría.</p>
        <div className="flex flex-wrap justify-center gap-4">
          <a href="mailto:carla@cardev.cl" className="bg-gradient-to-r from-cyan-500 to-purple-500 px-8 py-3 rounded-lg font-semibold hover:shadow-lg hover:shadow-cyan-500/50 transition">Enviar email</a>
          <a href="https://www.linkedin.com/in/carla-molina" target="_blank" rel="noopener noreferrer" className="border border-white/20 px-8 py-3 rounded-lg font-semibold hover:bg-white/5 transition flex items-center gap-2">LinkedIn <ExternalLink className="w-4 h-4" /></a>
        </div>
      </section>
